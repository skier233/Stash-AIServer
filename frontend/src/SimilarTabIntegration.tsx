    ...
    try {
      ...
      PluginApi.patch.before("ScenePage.Tabs", function (props: any) {
        try {
          ...
          function findNestedEventKey(c:any): string | undefined {
            try {
              if (!c || !c.props) return undefined;
              // Direct props.eventKey
              if (c.props.eventKey) return c.props.eventKey;
              // data-event-key variants
              if (c.props['data-event-key']) return c.props['data-event-key'];
              if (c.props['data-eventKey']) return c.props['data-eventKey'];
              ...
              for (const item of candidates) {
                ...
                if (item.props) {
                  ...
                  if (item.props['data-rb-event-key']) return item.props['data-rb-event-key'];
                  if (item.props['data-rb-eventKey']) return item.props['data-rb-eventKey'];
                  if (item.props['data-event-key']) return item.props['data-event-key'];
                  if (item.props.eventKey) return item.props.eventKey;
                }
              }
            } catch (e) { /* ignore */ }
            return undefined;
          ...
          const detailsIndex = childrenArray.findIndex((c:any) => {
          ...
          if (detailsIndex >= 0) {
            ...
            const queueIndex = childrenArray.findIndex((c:any) => {
              const ek = findNestedEventKey(c);
          ...
          if (insertIndex < 0) {
            ...
            insertIndex = childrenArray.findIndex((c: any) => {
              const ek = findNestedEventKey(c);
              return ek === targetEventKey;
            });
          }

          const navItem = React.createElement(Nav.Item, { key: 'similar-nav-item' },
            React.createElement(Nav.Link, { eventKey: "similar-tab", key: 'similar-nav-link' }, "Similar")
          );
      ...
      PluginApi.patch.before("ScenePage.TabContent", function (props: any) {
        try {
          ...
          const targetEventKey = 'scene-queue-panel';
          let insertIndex = childrenArray.findIndex((c: any) => c && c.props && (c.props.eventKey === targetEventKey || c.props['data-event-key'] === targetEventKey || c.props['data-eventKey'] === targetEventKey));
