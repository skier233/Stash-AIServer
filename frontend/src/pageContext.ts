...
function debugLog(...args: any[]) {
  if ((window as any).AIPageContextDebug) {
...
const PAGE_DEFS: Array<{
  key: PageContext['page'];
  segment: string;
  label: string;
  detailLabel: (id: string | null) => string;
}> = [
...
function collectSelectedIds(page: PageContext['page']): string[] | undefined {
  try {
  ...
  const m = href.match(/\/(scenes|performers|galleries|images|studios|tags|groups)\/(\d+)/);
    ...
    debugLog('collectSelectedIds', { page, count: finalIds?.length || 0, ids: finalIds });
    return finalIds;
  } catch {
    return undefined;
  }
...
export function subscribe(listener: (ctx: PageContext) => void) {
