...
const MinimalAIButton = () => {
    ...
    const backendBase = React.useMemo(() => getBackendBase(), []);
    ...
    const refetchActions = React.useCallback(async (ctx, opts = {}) => {
        ...
        try {
            const res = await fetch(`${backendBase}/api/v1/actions/available`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ context: { page: ctx.page, entityId: ctx.entityId, isDetailView: ctx.isDetailView, selectedIds: ctx.selectedIds || [] } }) });
            if (!res.ok)
    ...
    // Websocket ensure
    React.useEffect(() => { ensureTaskWebSocket(backendBase); }, [backendBase]);
    ...
    const toggleMenu = () => { if (!openMenu)
        refetchActions(context, { silent: true }); setOpenMenu((o) => !o); };
