...
@recommender(
    ...
    config=[
        ...
        { 'name':'focus_tags','label':'Focus Tags','type':'tags','default':[] },
...
async def baseline_popularity(ctx: Dict[str, Any], request: RecommendationRequest):
    ...
    # --- Apply config driven transformations (lightweight; avoid breaking pagination) ---
    min_score = cfg.get('min_score')
    if isinstance(min_score, (int, float)) and min_score > 0:
        # Filter in-place; if we drop items we won't backfill within this simple baseline.
    ...
    # 'pop' keeps fetch order (placeholder for real popularity ordering)
    ...
    if focus_tags or boost_performers:
        def _score(scene: Dict[str, Any]):
            ...
            tag_hits = len(tag_ids & focus_tags)
            perf_hits = len(performer_ids & boost_performers)
            return (tag_hits * 2) + perf_hits
        scenes.sort(key=_score, reverse=True)

    scoring_mode = cfg.get('scoring_mode') or 'simple'
    # Derive a simple score if weighted mode selected.
    ...
    note = cfg.get('note')
    # rank_window currently informational; could later drive overfetch logic
    ...
    for idx, sc in enumerate(scenes):
        ...
        if note:
            dm['note'] = note
        if focus_tags:
            dm['focus_tag_hit'] = bool(set(t.get('id') for t in sc.get('tags', [])) & focus_tags)
        if boost_performers:
            dm['performer_boost_hit'] = bool(set(p.get('id') for p in sc.get('performers', [])) & boost_performers)
        if rank_window:
            dm['rank_window'] = rank_window
