...
async def random_recent(ctx: Dict[str, Any], request: RecommendationRequest):
    ...
    shuffle_span_s = cfg.get('shuffle_span_s', 300)  # bucket duration controlling deterministic window
    overfetch_factor = max(1, int(cfg.get('overfetch_factor', 3)))
    recent_window = cfg.get('recent_window', '7d')  # placeholder; real impl would filter by added_at
    ...
    # Overfetch to allow better shuffle sample; rely on API meta from pagination util for total/has_more
    fetch_limit = limit * overfetch_factor
    scenes, approx_total, has_more = fetch_scenes_by_tag_paginated(932, offset, fetch_limit)

    # Filter out exclude_tags
    if exclude_tags:
        def _allowed(sc):
    ...
    # Pin performers to front while preserving relative order of rest
    if pin_performers:
        pinned: List[Dict[str, Any]] = []
        others: List[Dict[str, Any]] = []
        for sc in scenes:
            performer_ids = {p.get('id') for p in sc.get('performers', []) if isinstance(p, dict)}
            if performer_ids & pin_performers:
                pinned.append(sc)
            else:
                others.append(sc)
    ...
    if dedupe_studio:
        ...
        for sc in scenes:
            studio_id = sc.get('studio', {}).get('id') if isinstance(sc.get('studio'), dict) else None
            if studio_id and studio_id in seen_studios:
                continue
            if studio_id:
                seen_studios.add(studio_id)
            deduped.append(sc)
        scenes = deduped

    # Trim back to requested page size (since we over-fetched)
    if len(scenes) > limit:
        scenes = scenes[:limit]
    ...
    for idx, sc in enumerate(scenes):
        ...
        if seed_bucket is not None:
            dm['seed_bucket'] = seed_bucket
        dm['overfetch_factor'] = overfetch_factor
        dm['recent_window'] = recent_window
        if dedupe_studio:
            dm['deduped_studio'] = True
        if exclude_tags:
            dm['exclude_tags'] = list(exclude_tags)
        if pin_performers:
            dm['pin_performers'] = list(pin_performers)
        if debug_label:
            dm['label'] = debug_label
