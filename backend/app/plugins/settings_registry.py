...
# Shape for incoming definitions
# { key, type, default, label, description, options }
...
def register_settings(db: Session, plugin_name: str, definitions: Iterable[Mapping[str, Any]]):
    ...
    for d in definitions:
        ...
        if not row:
            ...
            # Update metadata if changed; keep user value unless None
            meta_changed = False
            if row.type != t:
                row.type = t; meta_changed = True
            if (row.label or '') != (label or ''):
                row.label = label; meta_changed = True
            if (row.description or '') != (desc or ''):
                row.description = desc; meta_changed = True
            if row.default_value != default:
                row.default_value = default; meta_changed = True
            if row.options != options:
                row.options = options; meta_changed = True
            if row.value is None and default is not None:
                row.value = default; meta_changed = True
            if meta_changed:
                changed = True
    if changed:
        db.commit()
